...
fun <V, K> DirectedEdgeView(
    ...
    var firstViewModel by remember { mutableStateOf(dirEdgeViewModel.firstVertexViewModel) }
    var secondViewModel by remember { mutableStateOf(dirEdgeViewModel.secondVertexViewModel) }
    ...
    Canvas (modifier = modifier.fillMaxSize()) {
        ...
        val path = Path().apply {
            val angle = (atan2(secondViewModel.x.toPx() - firstViewModel.x.toPx(), -secondViewModel.y.toPx() + firstViewModel.y.toPx())  - PI / 2).toFloat()
            val sin = sin(angle)
            val cos = cos(angle)
            moveTo(secondViewModel.x.toPx() + radius.toPx() - radius.toPx() * cos, secondViewModel.y.toPx() + radius.toPx() -radius.toPx() * sin)
            ...
            lineTo(secondViewModel.x.toPx() + radius.toPx() + firstCornerX, secondViewModel.y.toPx() + radius.toPx() + firstCornerY)
